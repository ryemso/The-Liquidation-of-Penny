// Chapter locations with per-room background overrides. Backgrounds never affect collision.
export const LOCATIONS = [
 ...
 {id:'pentagon-exterior',name:'펜타곤',fog:'#141e20',light:'#c3b17d',rooms:['외부 · 국가계약의 문','1층 · 조달과 예산','중앙회의실 가는 길','중앙회의실 · 최종 결재']}
];
export const locationFor = chapter => LOCATIONS[Math.max(0,Math.min(5,(chapter||1)-1))];
export const BACKGROUND_ASSETS=[...LOCATIONS.map(l=>l.id),'pentagon-lobby','pentagon-corridor','pentagon-chamber'];
...
export function drawLocation(ctx,images,spec,cam,time,cameraY=0){
 const theme=locationFor(spec.chapter),im=images[spec.background||theme.id],room=spec.localRoom||1;
