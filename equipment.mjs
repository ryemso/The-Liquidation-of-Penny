...
export function levelBonus(item,level=0){level=Number.isInteger(level)?Math.max(0,Math.min(5,level)):0;return Object.fromEntries(Object.entries(item.bonus).map(([k,v])=>[k,v>0?Math.round(v*(1+level*.1)*1000)/1000:v]));}
export function bonuses(value,levels={}){const sum={};for(const id of Object.values(normalizeLoadout(value))){const item=EQUIPMENT.find(e=>e.id===id);for(const [key,n] of Object.entries(item?levelBonus(item,levels[id]):{}))sum[key]=(sum[key]||0)+n;}return sum;}
export function bonusText(b){const labels={atk:'공격력',maxHp:'최대 체력',speed:'이동 속도',crit:'치명타 확률',armor:'피해 감소',dividend:'방 완료 회복',profitGain:'적중 수익',circuitExtra:'서킷 지속시간'};return Object.entries(b).map(([k,v])=>`${labels[k]} ${v>=0?'+':''}${['crit','armor'].includes(k)?Number((v*100).toFixed(1))+'%':v}${k==='circuitExtra'?'초':''}`).join(' · ')||'추가 보너스 없음';}
export function applyStartingEquipment(game,value,levels={}){game.equipmentLevels=Object.fromEntries(EQUIPMENT.map(e=>[e.id,Number.isInteger(levels[e.id])?Math.max(0,Math.min(5,levels[e.id])):0]));game.equipment=normalizeLoadout(value);for(const [key,n] of Object.entries(bonuses(game.equipment,game.equipmentLevels))){if(key==='maxHp'){game.player.maxHp+=n;game.player.hp+=n;}else game.stats[key]+=n;}}
