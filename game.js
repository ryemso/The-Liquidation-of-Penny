...
const equipmentUI=createEquipmentUI({wallet:pizzaWallet,onChange:updateWallet,storage:equipmentStorage,showModal,hideModal,getState:()=>game?.state});
...
function pauseGame(help=false){
 ...
 showModal(`<span class="eyebrow">${help?'HOW TO PLAY':'TRADING PAUSED'}</span><h2 id="modal-title">${help?'시장에서 살아남는 법':'잠시 거래를 멈춥니다'}</h2><div class="help-grid"><span><kbd>← →</kbd> 이동</span><span><kbd>C</kbd> 누르는 길이로 높이 조절 · 2단 점프 · ↓+C 내려가기</span><span><kbd>X</kbd> 3단 공격 · ↑+X 위 / 공중 ↓+X 아래</span><span><kbd>Z</kbd> 무적 대시</span><span><kbd>벽 방향 + C</kbd> 벽 점프 · 벽 방향 유지 시 미끄러짐</span><span><kbd>A</kbd> 수익 25 이상일 때 익절</span><span><kbd>S</kbd> 8초 레버리지</span><span><kbd>D</kbd> 적 정지 · 타격 35% 축적 후 추가 폭발</span><span><kbd>↑</kbd> 출구 / 거래소 이용</span></div><p>공격이 적중하면 미실현 수익이 쌓이고, 피격 시 25%를 잃습니다. 익절로 수익을 소모해 강한 공격을 쓰세요.<br>6장 집행관의 정책은 A/S/D의 필요 수익을 6초간 변경합니다. 버튼에 표시된 비용을 확인하세요. 익절은 보유 수익 전부를 소모하고, S/D는 표시 비용만 소모합니다.<br>레버리지는 피해 ×1.65. 8초 안에 2마리 처치 또는 피해 160을 달성하면 시드 +18, 실패하면 체력 −18입니다.</p>${!fromTitle?`<p>장착 장비 · ${Object.values(game.equipment).filter(Boolean).map(id=>EQUIPMENT.find(e=>e.id===id)?.name).join(' · ')||'없음'}<br>현재 포트폴리오 · ${buildText}</p>`:''}<div class="modal-actions">${!fromTitle?'<button class="secondary" id="restart-request">처음부터 다시</button>':''}<button class="primary" id="resume">${fromTitle?'확인':'계속하기'}</button></div>`,'pause');
...
function bind(){
 $('run-logs').onclick=exportRuns;window.addEventListener('pagehide',e=>{if(e.persisted)return;if(game?.log.started&&!game.log.ended){game.log.end('page_exit');archiveRun();}});
...
function enemyFrame(e){if(e.flash>0)return 7;if(e.type==='algorithm')return e.state==='windup'?2:e.state==='attack'?3:Math.floor(e.anim*2)%2;if(e.type==='boss'||e.type==='enforcer'||e.type==='central')return e.state==='windup'?1:e.state==='attack'?2:e.state==='recover'?3:0;if(e.type==='shield')return e.state==='attack'||e.state==='windup'?2:e.state==='recover'?3:Math.abs(e.vx)>1?1:0;if(e.type==='drone')return e.state==='windup'?2:e.state==='recover'?3:Math.floor(e.anim*2)%2;if(e.type==='boss')return e.state==='windup'?1:e.state==='attack'?2:0;if(e.state==='windup')return 4;if(e.state==='attack')return 5;if(e.state==='charge')return Math.floor(e.anim*10)%2?5:6;if(e.state==='recover')return 6;if(Math.abs(e.vx)>5||e.type==='ghost')return 2+Math.floor(e.anim*7)%2;return Math.floor(e.anim*2)%2;}
function drawEnemy(e){if(e.dead)return;const x=e.x+e.w/2,bottom=e.y+e.h;const height=e.type==='algorithm'?138:e.type==='boss'||e.type==='enforcer'||e.type==='central'||e.type==='market'?178:e.type==='ghost'?96:e.type==='drone'?70:e.type==='shield'?82:e.type==='bomb'?74:e.elite?98:76;ctx.save();ctx.globalAlpha=.3;ctx.fillStyle='#02060c';ctx.beginPath();ctx.ellipse(x,e.type==='ghost'?620:bottom,e.w*.65,5,0,0,Math.PI*2);ctx.fill();ctx.restore();
 if(e.state==='windup'&&!e.boss){const factor=1-(Math.max(0,e.timer)/(e.type==='boss'?1.15:e.type==='bomb'?1.15:.65));ctx.save();ctx.strokeStyle=e.type==='ghost'?'#78c6eaaa':'#f9b870bb';ctx.lineWidth=2;ctx.beginPath();ctx.arc(x,e.y+e.h*.4,Math.max(e.w*.7,27)+Math.sin(game.t*20)*2,-Math.PI/2,Math.PI*2*factor-Math.PI/2);ctx.stroke();ctx.restore();label('!',x,e.y-28,'#ffd185',22);if(e.type==='bomb'){ctx.fillStyle='#df6d4222';ctx.beginPath();ctx.ellipse(x,620,133,13,0,0,Math.PI*2);ctx.fill();}}
 const skinPrefix=(game.spec.chapter===2||game.spec.chapter===6)?'inst':game.spec.chapter===3?'algo':game.spec.chapter===4?'bank':game.spec.chapter===5?'market':null;
...
function drawHero(){const p=game.player;let frame=0;if(p.hurt>0)frame=7;else if(p.wallSliding)frame=5;else if(p.attackMove)frame=p.attackMove.elapsed<.035?0:p.attackMove.elapsed<.14?6:0;else if(!p.grounded)frame=p.vy<0?4:5;else if(Math.abs(p.vx)>10)frame=1+Math.floor(game.t*11)%3;
 ...
 if(p.leverage>0&&Math.abs(p.vx)>10){for(let i=2;i>0;i--)sprite('hero',frame,x-p.facing*i*12,bottom,45.22,p.facing,0,.12);}
...
function drawEffects(){
 drawCombatFeel(ctx,game,reducedMotion);
 for(const h of game.hazards){if(h.delay>0){if(['boss','enforcer','algorithm','central','market'].includes(h.source?.type))continue;rect(h.x,614,h.w,6,Math.sin(game.t*20)>0?'#ef8376':'#a54a4b');rect(h.x,455,h.w,160,'#e97b6814');label('!',h.x+h.w/2,600,'#ffb294',20);}else{const alpha=Math.min(1,h.life/.45);ctx.save();ctx.globalAlpha=alpha;rect(h.x,455,h.w,165,'#eab07b70');rect(h.x+20,455,h.w-40,165,'#ffd992');ctx.restore();}}
...
function render(){
 ...
 drawTerrain(ctx,game.terrain,game.spec.chapter,game.t);drawExploration(ctx,game);
 ...
 ctx.restore();
 if(game.conceptCard?.remaining>0){const c=game.conceptCard;rect(245,105,790,84,'#101923ee');label(c.title+' · 게임적 비유',640,127,'#e7c892',15);c.lines.forEach((line,i)=>label(line,640,149+i*19,'#ccd4db',12));}
...
function updateHUD(){totemHUD(game,$('totem-hud'));const p=game.player;$('hp-fill').style.width=`${p.hp/p.maxHp*100}%`;$('hp-text').textContent=`${Math.ceil(p.hp)} / ${p.maxHp}`;$('profit-fill').style.width=`${p.profit}%`;$('profit-text').textContent=`${Math.floor(p.profit)}%`;$('gold').textContent=String(p.gold);$('room-number').textContent=`${String(game.spec.localRoom).padStart(2,'0')} / 05`;$('rank').textContent=game.state==='victory'?'SMALL CAP':'PENNY STOCK';const m=MARKETS[game.market];$('market-name').textContent=m.name;$('market-name').style.color=m.color;$('market-detail').textContent=m.detail;$('market-warning').textContent=game.marketClock>game.marketPeriod-4?`${Math.ceil(game.marketPeriod-game.marketClock)}초 후 ${MARKETS[(game.market+1)%3].name}`:'';
 ...
 for(const [name,cd,desc]of skillList){const b=$(`skill-${name}`);b.classList.toggle('cooldown',cd>0);const cost=skillCost(game,name);b.classList.toggle('ready',cd<=0&&p.profit>=cost);b.querySelector('small').textContent=(cd>0?`${Math.ceil(cd)}초 · `:'')+(name==='profit'?`수익 ${cost} 이상`:cost>0?`수익 ${cost} 소모`:desc);}
