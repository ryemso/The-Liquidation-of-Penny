...
function event(e){
 ...
 if(e.type==='chapter'){stageMusic.setScene('chapter',game.spec.chapter);const next=game.spec.chapter+1;showModal(`<span class="eyebrow">CHAPTER ${game.spec.chapter} COMPLETE</span><h2 id="modal-title">${next===2?'기관의 벽':'알고리즘의 심장'}으로</h2><p>최대 체력 +20 · 공격력 +6 · 체력 45 회복 · 수익 50으로 다음 장을 시작합니다.</p><div class="modal-actions"><button class="primary" id="enter-chapter2">${next}장 진입 →</button></div>`,'chapter');$('enter-chapter2').onclick=()=>game.enterNextChapter();}
 ...
 if(e.type==='finish'){archiveRun();stageMusic.setScene('title',game.spec.chapter);
  ...
  const won=e.won;
  showModal(`<span class="eyebrow">${won?'LISTING APPROVED · 상장 승인':'LIQUIDATED · 이번 도전 종료'}</span><h2 id="modal-title">${won?'Mid Cap → Blue Chip':'청산되었습니다'}</h2><p>${won?'알고리즘 코어를 돌파했습니다. 1~3장 도전을 완료하고 Blue Chip으로 승격했습니다.':'이번 손실도 다음 판단의 근거가 됩니다. 투자 지식은 다음 도전에 남습니다.'}</p><div class="stat-grid"><div><strong>${fmtTime(game.totalTime)}</strong><small>플레이 시간</small></div><div><strong>${game.kills}</strong><small>처치한 적</small></div><div><strong>+${e.knowledge}</strong><small>투자 지식</small></div></div><p>이번 도전 · 공격 ${game.log.export().summary.attacks}회 · 토템 발동 ${game.log.export().summary.totem_activations}회<br>보유 투자 지식 ${progress.knowledge} · 다음 시작 체력 ${100+Math.min(20,progress.knowledge*2)}<br>이번 포트폴리오: ${game.build.length?game.build.map(id=>CARDS.find(c=>c.id===id).name).join(' · '):'없음'}</p><div class="modal-actions"><button class="secondary" id="back-title">처음으로</button><button class="primary" id="retry">다시 도전 ↗</button></div>`,'finish');$('retry').onclick=startGame;$('back-title').onclick=backToTitle;
 }
}
...
function enemyFrame(e){if(e.flash>0)return 7;if(e.type==='algorithm')return e.state==='windup'?2:e.state==='attack'?3:Math.floor(e.anim*2)%2;if(e.type==='boss'||e.type==='enforcer')return e.state==='windup'?1:e.state==='attack'?2:e.state==='recover'?3:0;if(e.type==='shield')return e.state==='attack'||e.state==='windup'?2:e.state==='recover'?3:Math.abs(e.vx)>1?1:0;if(e.type==='drone')return e.state==='windup'?2:e.state==='recover'?3:Math.floor(e.anim*2)%2;if(e.type==='boss')return e.state==='windup'?1:e.state==='attack'?2:0;if(e.state==='windup')return 4;if(e.state==='attack')return 5;if(e.state==='charge')return Math.floor(e.anim*10)%2?5:6;if(e.state==='recover')return 6;if(Math.abs(e.vx)>5||e.type==='ghost')return 2+Math.floor(e.anim*7)%2;return Math.floor(e.anim*2)%2;}
function drawEnemy(e){if(e.dead)return;const x=e.x+e.w/2,bottom=e.y+e.h;const height=e.type==='algorithm'?138:e.type==='boss'||e.type==='enforcer'?178:e.type==='ghost'?96:e.type==='drone'?70:e.type==='shield'?82:e.type==='bomb'?74:e.elite?98:76;ctx.save();ctx.globalAlpha=.3;ctx.fillStyle='#02060c';ctx.beginPath();ctx.ellipse(x,e.type==='ghost'?620:bottom,e.w*.65,5,0,0,Math.PI*2);ctx.fill();ctx.restore();
 ...
 sprite(e.type==='algorithm'?'drone':e.type,enemyFrame(e),x,bottom,height,e.type==='boss'?-e.facing:e.facing,e.flash);
