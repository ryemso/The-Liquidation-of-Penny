...
function event(e){
 ...
 if(e.type==='finish'){archiveRun();stageMusic.setScene('title',game.spec.chapter);
  ...
  const won=e.won;
  showModal(`<span class="eyebrow">${won?'LISTING APPROVED · 상장 승인':'LIQUIDATED · 이번 도전 종료'}</span><h2 id="modal-title">${won?'Blue Chip → Mega Cap':'청산되었습니다'}</h2><p>${won?'통화정책 위원회를 돌파했습니다. 1~4장 도전을 완료하고 Mega Cap으로 승격했습니다.':'이번 손실도 다음 판단의 근거가 됩니다. 투자 지식은 다음 도전에 남습니다.'}</p><div class="stat-grid"><div><strong>${fmtTime(game.totalTime)}</strong><small>플레이 시간</small></div><div><strong>${game.kills}</strong><small>처치한 적</small></div><div><strong>+${e.knowledge}</strong><small>투자 지식</small></div></div><p>이번 도전 · 공격 ${game.log.export().summary.attacks}회 · 토템 발동 ${game.log.export().summary.totem_activations}회<br>보유 투자 지식 ${progress.knowledge} · 다음 시작 체력 ${100+Math.min(20,progress.knowledge*2)}<br>이번 포트폴리오: ${game.build.length?game.build.map(id=>CARDS.find(c=>c.id===id).name).join(' · '):'없음'}</p><div class="modal-actions"><button class="secondary" id="back-title">처음으로</button><button class="primary" id="retry">다시 도전 ↗</button></div>`,'finish');$('retry').onclick=startGame;$('back-title').onclick=backToTitle;
 }
}
...
function drawEnemy(e){if(e.dead)return;const x=e.x+e.w/2,bottom=e.y+e.h;const height=e.type==='algorithm'?138:e.type==='boss'||e.type==='enforcer'||e.type==='central'?178:e.type==='ghost'?96:e.type==='drone'?70:e.type==='shield'?82:e.type==='bomb'?74:e.elite?98:76;ctx.save();ctx.globalAlpha=.3;ctx.fillStyle='#02060c';ctx.beginPath();ctx.ellipse(x,e.type==='ghost'?620:bottom,e.w*.65,5,0,0,Math.PI*2);ctx.fill();ctx.restore();
 ...
 const bankSkin=game.spec.chapter>=4&&['shield','drone','bomb','ghost','central'].includes(e.type);
 const bankFrame=e.state==='windup'?1:(e.state==='attack'||e.state==='charge')?2:e.state==='recover'?3:0;
 sprite(bankSkin?'bank-'+e.type:e.type==='central'?'enforcer':e.type==='algorithm'?'drone':e.type,bankSkin?bankFrame:enemyFrame(e),x,bottom,height*(bankSkin&&e.elite?1.15:1),e.type==='boss'?-e.facing:e.facing,e.flash);
