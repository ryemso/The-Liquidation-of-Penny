...
function drawEnemy(e){if(e.dead)return;const x=e.x+e.w/2,bottom=e.y+e.h;const height=e.type==='algorithm'?138:e.type==='boss'||e.type==='enforcer'||e.type==='central'||e.type==='market'?178:e.type==='ghost'?96:e.type==='drone'?70:e.type==='shield'?82:e.type==='bomb'?74:e.elite?98:76;ctx.save();ctx.globalAlpha=.3;ctx.fillStyle='#02060c';ctx.beginPath();ctx.ellipse(x,e.type==='ghost'?620:bottom,e.w*.65,5,0,0,Math.PI*2);ctx.fill();ctx.restore();
 ...
 sprite(bankSkin?skinPrefix+'-'+e.type:e.type==='central'?'enforcer':e.type==='algorithm'?'drone':e.type,bankSkin?bankFrame:enemyFrame(e),x,bottom,height*(bankSkin&&e.elite?1.15:1),e.type==='boss'?-e.facing:e.facing,e.flash);
