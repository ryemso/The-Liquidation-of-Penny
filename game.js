...
async function init(){
 ...
 for(const [prefix,file,boss,cuts] of [['algo','algorithm-monsters.png','algorithm',[0,.194,.356,.537,.725,1]],['market','market-monsters.png','market',[0,.209,.383,.543,.709,1]]]){const sheet=await imageLoad('./'+file),types=['shield','drone','bomb','ghost',boss];for(let row=0;row<5;row++){const strip=document.createElement('canvas'),top=Math.round(cuts[row]*sheet.height);strip.width=sheet.width;strip.height=Math.round(cuts[row+1]*sheet.height)-top;strip.getContext('2d').drawImage(sheet,0,top,sheet.width,strip.height,0,0,sheet.width,strip.height);assets[prefix+'-'+types[row]]=atlas(strip,4,1,false);}}
