...
test('attack needs a directional hitbox; profit consumes accumulated value',()=>{
 const g=game();step(g,.3);g.player.x=600;g.player.facing=1;g.enemies=[g.makeEnemy('rubble',665,620)];const e=g.enemies[0];g.action('attack');assert.equal(e.hp,e.maxHp);step(g,.05);assert.ok(e.hp<e.maxHp);assert.equal(g.player.profit,8);g.player.profit=60;g.action('profit');assert.equal(g.player.profit,0);assert.ok(g.player.profitCD>0);assert.equal(e.dead,true);
});
