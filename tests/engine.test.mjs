...
test('attack needs a directional hitbox; profit consumes accumulated value',()=>{
 const g=game();step(g,.3);g.player.x=600;g.player.facing=1;g.enemies=[g.makeEnemy('rubble',665,620)];const e=g.enemies[0];g.action('attack');assert.equal(e.hp,e.maxHp);step(g,.05);assert.ok(e.hp<e.maxHp);assert.equal(g.player.profit,7);g.player.profit=60;g.action('profit');assert.equal(g.player.profit,0);assert.ok(g.player.profitCD>0);assert.equal(e.dead,true);
});
...
test('room cannot be skipped before clear and a reward unlocks exactly the next room',()=>{
 const g=game();g.setRoom(3);g.player.x=g.width-80;g.action('interact');assert.equal(g.room,3);g.enemies.forEach(e=>e.dead=true);step(g,1);assert.equal(g.state,'reward');g.chooseReward('growth');assert.equal(g.stats.atk,23);g.action('interact');assert.equal(g.room,4);assert.equal(g.doorOpen,false);
});
...
test('boss attacks without instructional warning notices and defeating it opens the chapter completion gate',()=>{
