...
test('self destruction records defeat only once',()=>{const g=make();const e=g.makeEnemy('bomb',1500,620);g.enemies=[e];g.explode(e);g.explode(e);assert.equal(g.kills,1);assert.equal(g.log.events.filter(x=>x.event==='enemy_defeated').length,1);});
